...
namespace webhook {
  ...
  enum class format_type_t {
    MARKDOWN,     // Markdown (supports HTML tags)
    TEXT,         // Plain text
    JSON,         // JSON
  ...
  /**
   * @brief Webhook format configuration.
   */
  class WebhookFormat {
  public:
    /**
    ...
    /**
     * @brief Set whether colors are used.
     * @param use_colors Whether to use colors.
     */
    void set_use_colors(bool use_colors);

    /**
     * @brief Set whether IP addresses are simplified.
     * @param simplify_ip Whether to simplify IP addresses.
     */
     ...
     */
    std::string generate_content(const event_t& event) const;

    /**
     * @brief Generate the complete JSON payload.
     * @param event Event data.
     * @return JSON string.
     */
    std::string generate_json_payload(const event_t& event) const;

    /**
     * @brief Generate a test payload using the same envelope as production notifications.
     * @return JSON string.
     */
  ...
  private:
     ...
     * @param timestamp Sunshine local time format (YYYY-MM-DD HH:mm:ss.xxx).
     ...
     * @return Event title.
     */
    std::string get_event_title(event_type_t event_type) const;

    /**
     * @brief Generate Markdown content.
     * @param event Event data.
     * @return Markdown content.
     */
    std::string generate_markdown_content(const event_t& event) const;

    /**
     * @brief Generate plain text content.
     * @param event Event data.
     * @return Text content.
     */
    std::string generate_text_content(const event_t& event) const;

    /**
     * @brief Generate JSON content.
     * @param event Event data.
     * @return JSON content.
     */
    std::string generate_json_content(const event_t& event) const;

    /**
     * @brief Generate custom-template content.
     * @param event Event data.
     * @return Custom content.
     */
    std::string generate_custom_content(const event_t& event) const;
   ...
   * @param use_markdown Whether to use the Markdown format (default true).
   */
  void configure_webhook_format(bool use_markdown = true);

  /**
   * @brief Validate that the content length meets the webhook requirements.
   * @param content Content string.
   * @return Whether the length requirement is met.
   */
