  ...
  display_ddup_vram_t::snapshot(const pull_free_image_cb_t &pull_free_image_cb, std::shared_ptr<platf::img_t> &img_out, std::chrono::milliseconds timeout, bool cursor_visible) {
    ...
    if (shape_updated) {
      ...
      if (!normalize_cursor_shape(
        ...
        // A shape we cannot normalize is a data problem, not a device state problem,
        // so it stays a hard error and is bounded by the capture retry window.
        BOOST_LOG(error) << "Failed to normalize the cursor shape"sv;
        return capture_e::error;
      }
